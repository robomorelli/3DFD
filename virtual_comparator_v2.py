...
def find_zero_point_flatness(rivet_center, hole_radius, pts, kdtree,
    ...
    For each candidate in the search band, roughness = std of residuals from
    a locally-fitted plane within flatness_radius.  The flattest vertex is
    returned as the zero point.

    Candidates are subsampled to max_candidates using angular stratification
    (covers all directions evenly) before computing roughness, so performance
    is independent of mesh density.

    Returns (zero_point, roughness_value) or (None, inf).
    """
    ...
    best = int(np.argmin(roughness))
    if not np.isinf(roughness[best]):
        return pts[idxs[best]].copy(), float(roughness[best])
    return None, float("inf")
