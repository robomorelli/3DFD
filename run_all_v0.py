...
def parse_args():
    ...
    p.add_argument("--out-dir",    default="comparator_output", help="Output directory")
...
def process_one(ply_path, out_dir, feet_radius, probe_radius,
    ...
    holes = find_holes(faces, pts, r_min=hole_r_min, r_max=hole_r_max)
    if not holes:
        return {"label": label, "n_holes": 0, "n_defects": 0,
                "worst_dev": None, "rows": [], "elapsed": time.time()-t0}

    results = []
    for i, hole in enumerate(holes):
        res = measure(hole["center"].copy(), pts, kdtree, feet_radius, probe_radius)
        if res is None:
            continue
        res["hole_idx"]  = i
        res["hole_r"]    = hole["radius_mm"]
        res["feet_r"]    = feet_radius
        res["probe_r"]   = probe_radius
        res["corrected"] = res["min_dist"]   # no zero offset in batch mode
        results.append(res)
    ...
    if results and save_plot:
        ...
        make_static_plot(pts, holes, results, 0.0, threshold, label, out_png)

    if results and save_html:
        out_html = os.path.join(out_dir, f"{label}_comparator.html")
        make_interactive_plot(pts, results, 0.0, threshold, label, out_html)

    rows = [{
        "surface":   label,
        "hole_idx":  r["hole_idx"] + 1,
        "hole_r_mm": round(r["hole_r"], 3),
        "min_dev_mm": round(r["corrected"], 4),
        "mean_dev_mm": round(r["mean_dist"], 4),
        "p5_dev_mm":  round(r["p5_dist"], 4),
        "n_probe_pts": r["n_probe"],
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as ex:
        ...
        for fut in as_completed(futures):
            ...
            print(f"  [{done:>2}/{len(ply_files)}] {res['label']:<25}  "
                  f"{res['n_holes']:>3} holes  {res['elapsed']:>5}s{flag}")
