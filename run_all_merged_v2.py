"""
Merged run with virtual_comparator_v2 (local-flatness zeroing).

Same file selection as run_all_merged.py (mesh3D preferred over pc),
same measurement parameters, same double-row logic — only the zeroing
method changes: flatness-based (min local roughness) instead of
deviation-map-based (min global residual).

Results go to comparator_output_merged_v2/ for direct comparison with
comparator_output_merged/.

Usage:
  python run_all_merged_v2.py
  python run_all_merged_v2.py --out-dir comparator_output_merged_v2 --workers 6
  python run_all_merged_v2.py --zero-flatness-radius 4.0
"""
...
# ── File-list builder (same logic as run_all_merged.py) ──────────────────────
...
def _build_file_list(mesh_dir="data/mesh3D", pc_dir="data/pc"):
    ...
    print(f"  mesh3D surfaces : {len(mesh_files)}")
    print(f"  pc-only surfaces: {len(pc_only)}")
    print(f"  total           : {len(mesh_files) + len(pc_only)}")
...
def parse_args():
    ...
    p.add_argument("--out-dir",   default="comparator_output_merged_v2")

    # Geometry (same defaults as run_all_autozero.py)
    ...
    p.add_argument("--zero-flatness-radius", type=float, default=3.0,
                   help="Radius (mm) of local neighbourhood for roughness computation")
    p.add_argument("--zero-max-candidates",  type=int,   default=256,
                   help="Max zero candidates after angular stratification")
    ...
    # Measurement mode
    p.add_argument("--measure-mode", choices=["plane", "local-poly", "per-point"],
                   default="plane")
...
def process_one(ply_path, out_dir, args_dict):
    ...
    from virtual_comparator_v2 import (
        find_zero_point_flatness, zero_reading_at,
    ...
    # ── Pass 1: find zero point for every rivet (flatness-based) ─────────────
    zero_pts_all   = []
    zero_rough_all = []
    for i, hole in enumerate(holes):
        ...
        zpt, zrq = find_zero_point_flatness(
            c, r, pts, kdtree,
            from_edge_min=z_edge_min,
            from_edge_max=z_edge_max,
            ...
            zero_search=z_search,
            crown_buffer=a["r_outer"],
            flatness_radius=a["zero_flatness_radius"],
            max_candidates=a["zero_max_candidates"],
        )
        zero_pts_all.append(zpt)
        zero_rough_all.append(zrq)

    # ── Double-row zero sharing (same logic as run_all_autozero.py) ───────────
    processed_pairs = set()
    for i in range(len(holes)):
        ...
        zpi, zpj = zero_pts_all[i], zero_pts_all[j]
        ci, cj   = centers_xy[i], centers_xy[j]
        dir_ij   = cj - ci
        ...
        if zpi is not None and zpj is not None:
            ...
            if dot_i < 0:
                zero_pts_all[j]   = zpi
                zero_rough_all[j] = zero_rough_all[i]
            else:
                zero_pts_all[i]   = zpj
                zero_rough_all[i] = zero_rough_all[j]
        elif zpi is not None:
            zero_pts_all[j]   = zpi
            zero_rough_all[j] = zero_rough_all[i]
        elif zpj is not None:
            zero_pts_all[i]   = zpj
            zero_rough_all[i] = zero_rough_all[j]

    # ── Zero propagation ──────────────────────────────────────────────────────
    last_seen_pt  = None
    last_seen_rq  = float("inf")
    pending       = []
    for i in range(len(zero_pts_all)):
        if zero_pts_all[i] is not None:
            last_seen_pt  = zero_pts_all[i]
            last_seen_rq  = zero_rough_all[i]
            for j in pending:
                zero_pts_all[j]   = last_seen_pt
                zero_rough_all[j] = last_seen_rq
            pending.clear()
        elif last_seen_pt is not None:
            zero_pts_all[i]   = last_seen_pt
            zero_rough_all[i] = last_seen_rq
        else:
            pending.append(i)

    # ── Pass 2: measure each rivet ────────────────────────────────────────────
    results = []
    for i, hole in enumerate(holes):
        c = hole["center"].copy()
        r = hole["radius_mm"]
        ...
        if (a.get("max_crown_dev_range") is not None
                and math.isfinite(res.get("crown_dev_range", float("nan")))
                and res["crown_dev_range"] > a["max_crown_dev_range"]):
            continue

        res["hole_idx"]       = i
        res["zero_center"]    = zero_pt
        res["zero_roughness"] = zero_rough_all[i]
        results.append(res)
    ...
    if a["plots"]:
        out_png = os.path.join(out_dir, f"{label}_comparator_v2.png")
    ...
    if a["interactive"]:
        out_html = os.path.join(out_dir, f"{label}_comparator_v2.html")
        make_interactive_plot(pts, holes, mastics, results,
                              a["threshold"], label, out_html, plot_args)

    csv_path = os.path.join(out_dir, f"{label}_comparator_v2.csv")
    ...
    rows = [{
        ...
        "zero_roughness_mm":   round(res["zero_roughness"], 5) if (res.get("zero_roughness") is not None and math.isfinite(res["zero_roughness"])) else "",
...
def summary_plot(summary_rows, threshold, warn_lo, warn_hi, out_path, critical_hi=0.60):
    ...
    fig.suptitle(
        f"Virtual Comparator v2 (flatness zeroing) — All surfaces summary\n"
        f"orange [{-warn_hi:.2f},{-warn_lo:.2f}]  ·  red [{-critical_hi:.2f},{-warn_hi:.2f}]  ·  black <{-critical_hi:.2f} mm",
        fontsize=13, fontweight="bold",
    )

    ax = axes[0]
    ax.bar(x, [r["n_holes"]   for r in rows], label="total holes",    color="steelblue", alpha=0.6)
    ax.bar(x, [r["n_defects"] for r in rows], label="flagged defects", color="tomato",   alpha=0.9)
    ax.set_xticks(x)
    ...
    ax2.axhline(-critical_hi, color="black",     ls="-",  lw=1.5, label=f"critical  {-critical_hi:.2f} mm")
    ax2.axhline(-warn_hi,     color="red",        ls="-",  lw=1.5, label=f"alarm     {-warn_hi:.2f} mm")
    ax2.axhline(-warn_lo,     color="darkorange", ls="-",  lw=1.2, label=f"warn      {-warn_lo:.2f} mm")
    ax2.set_xticks(x)
    ax2.set_xticklabels(labels, rotation=60, ha="right", fontsize=7)
    ax2.set_ylabel("worst k_worst_mean (mm)")
    ax2.set_title("Worst pull-in per surface  (k_worst_mean metric, v2 flatness zeroing)")
...
def main():
    ...
    print(f"Zero: flatness_radius={args.zero_flatness_radius} mm   "
          f"edge [{args.zero_from_edge_min:.0f}..{args.zero_from_edge_max:.0f}] mm   "
          f"max_candidates={args.zero_max_candidates}")
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as ex:
        ...
        for fut in as_completed(futures):
            ...
            print(f"  [{done:>2}/{len(file_list)}] {res['label']:<28}  "
